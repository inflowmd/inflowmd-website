...
/**
 * Vein Specialists — confidential prospect audit.
 *
 * Third in the series after /audit/vein-ity and /audit/veinandmedspa, and it
 * reuses the second one's components verbatim: the dark hero, PSI score
 ...
 * tokens, no new CSS.
 *
 * Two structural differences from that page, both deliberate:
 *
 *  - No competitor table and no local pack. The capture was captcha-blocked,
 *    so there is no verified pack — and an approximated one on a page whose
 *    credibility rests on verification would be worse than none.
 *  - No dual-build panel. That practice ran two service lines; this one runs
 *    a single line and a single patient journey.
 *
 * Hidden like the others: noindex, nofollow, no sitemap entry, linked from
 * nowhere.
 */

const canonical = "https://www.inflowmd.com/audit/veinspecialists";
...
/** The paid campaign, filed beside the speed finding it runs into. */
function AdsBlock() {
  return (
    <div className="rounded-3xl border border-white/10 bg-dark-card shadow-xl p-6 sm:p-9">
      ...
      <div className="mt-7 rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
        ...
        <ul>
          {ads.map.map((row) => (
            <li
              key={row.promise}
              className="grid sm:grid-cols-[minmax(0,16rem)_1fr] gap-2 sm:gap-6 px-4 sm:px-6 py-4 border-b border-white/10 last:border-b-0"
            >
      ...
      <div className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-5 sm:p-6">
        <div className="text-[10px] font-bold tracking-[0.2em] uppercase text-gray-500 mb-2">
          {ads.advertiser.title}
        </div>
        <p className="text-gray-200 text-sm sm:text-base leading-relaxed">{ads.advertiser.body}</p>
      </div>
...
/* No slot component here: this page has no awaiting-data blocks — every
   figure on it is verified or absent. */
...
export default function VeinSpecialistsAuditPage() {
  return (
          ...
          <div className="rounded-3xl border border-white/10 bg-dark-card shadow-xl p-6 sm:p-8">
            ...
            <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,20rem)_1fr] lg:items-start">
              <div>
                    ...
                    >
                      <div
                        className={`text-2xl sm:text-3xl font-extrabold tabular-nums leading-none ${
                          Number(sc.value) >= 90 ? "text-emerald-400" : "text-red-400"
                        }`}
                      >
                        {sc.value}
                      </div>
                ...
                <p className="mt-3 text-emerald-300 text-sm leading-snug font-semibold">
                  {summary.scoresNote}
                </p>
              </div>
      ...
      <main>
        ...
        <section id="website" className="bg-warm-bg py-16 sm:py-24 scroll-mt-16 border-y border-black/5">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            <div className="rounded-3xl border border-white/10 bg-dark-card shadow-xl p-6 sm:p-10">
              ...
              {/* The crown, given its own frame directly under the rings. */}
              <div className="mt-10 rounded-2xl border-2 border-emerald-400/40 bg-emerald-500/[0.08] p-5 sm:p-7">
                <p className="text-gray-100 text-base sm:text-lg leading-relaxed max-w-3xl">
                  <RichText text={engine.crown} />
                </p>
              </div>
            ...
            {/* Critical strip — plain sentences, no cards. */}
            ...
            <div className="mt-6 space-y-4">
              {websiteFindings.map((f) => (
                <div key={f.id} className="space-y-4">
                  <Finding f={f} light />
                  {/* The campaign runs into the finding above it. */}
                  {f.id === "screening-cta" && <AdsBlock />}
                </div>
              ))}
            </div>

            {/* Crowns — named specifically, and generously. */}
            <div className="mt-10 rounded-3xl border-2 border-emerald-300 bg-emerald-50 p-6 sm:p-8">
              <div className="text-[10px] font-bold tracking-[0.22em] uppercase text-emerald-800 mb-3">
                {crowns.title}
              </div>
              <p className="text-slate-700 text-base sm:text-lg leading-relaxed max-w-3xl mb-6">
                {crowns.lead}
              </p>
              <ul className="grid sm:grid-cols-2 gap-x-8 gap-y-5">
                {crowns.items.map((c) => (
                  <li key={c.title} className="flex items-start gap-3">
                    ...
                    <span>
                      <span className="block text-slate-900 font-bold text-sm sm:text-base leading-snug">
                        {c.title}
                      </span>
                      <span className="block text-slate-700 text-sm leading-relaxed mt-1">
                        {c.detail}
                      </span>
                    </span>
                  </li>
        ...
        <section id="presence" className="bg-dark py-16 sm:py-24 scroll-mt-16">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            <SubHeading>{npi.title}</SubHeading>
            <p className="text-gray-300 text-base sm:text-lg leading-relaxed max-w-3xl">{npi.body}</p>
            <div className="mt-6 space-y-4">
              {npi.findings.map((f) => (
            ...
            <div className="rounded-2xl border-2 border-emerald-400/40 bg-emerald-500/[0.08] p-5 sm:p-7">
              <p className="text-gray-100 text-base sm:text-lg leading-relaxed max-w-3xl">
                <RichText text={reviews.crown} />
              </p>
            </div>
            <DataTable table={reviews.table} />
            <div className="mt-6 space-y-4">
              {reviews.findings.map((f) => (
                <Finding key={f.id} f={f} />
              ))}
            </div>
        ...
        <section id="plan" className="bg-warm-bg py-16 sm:py-24 scroll-mt-16 border-y border-black/5">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            <blockquote className="mt-6 rounded-2xl border border-slate-200 bg-white shadow-sm p-6 sm:p-8 max-w-3xl">
              <p className="text-slate-900 text-lg sm:text-xl md:text-2xl font-bold leading-snug">
                “{thesis.quote}”
              </p>
            </blockquote>
            {thesis.body.map((p, i) => (
            ...
            <div className="mt-8 grid sm:grid-cols-3 gap-4">
              {thesis.proofPoints.map((pp) => (
                <div key={pp.label} className="rounded-2xl border border-slate-200 bg-white shadow-sm p-5">
                  <div className="text-slate-900 font-extrabold text-lg leading-snug">{pp.label}</div>
                  <div className="text-slate-600 text-sm leading-relaxed mt-1.5">{pp.detail}</div>
                </div>
              ))}
            </div>
            <p className="text-slate-700 text-base sm:text-lg leading-relaxed max-w-3xl mt-6">
              {thesis.closing}
            </p>
            ...
            <div className="mt-20 border-t border-black/10 pt-14">
              ...
              <div className="space-y-5">
                {plan.phases.map((phase, i) => (
                  ...
                  >
                    ...
                    <p className="mt-4 text-accent text-base sm:text-lg font-semibold leading-snug">
                      {phase.outcome}
                    </p>
              ...
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed max-w-3xl mt-4">
                {plan.guarantee}
              </p>
            </div>
          </div>
        ...
        <section id="investment" className="bg-dark py-16 sm:py-24 scroll-mt-16">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            <div className="mt-8 rounded-3xl border-2 border-emerald-400/40 bg-emerald-500/[0.08] p-6 sm:p-9">
              <div className="text-[10px] font-bold tracking-[0.22em] uppercase text-emerald-300 mb-4">
                {investment.pattyTitle}
              </div>
              {investment.patty.map((p, i) => (
                <p key={i} className="text-gray-100 text-base sm:text-lg leading-relaxed max-w-3xl mt-4 first:mt-0">
                  <RichText text={p} />
                </p>
              ))}
            </div>
